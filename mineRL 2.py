...
for episode in range(num_episodes):
    ...
    while True:
        ...
        cameraX = camerax_dist.sample()*3
        cameraY = cameray_dist.sample()

        cameraX = torch.clamp(cameraX, -3, 3)
        cameraY = torch.clamp(cameraY, -1, 1)
        ...
        prev_sum_inventory = sum(prev_inventory.values())
        current_sum_inventory = sum(next_state['inventory'].values())
        reward_diff = abs(prev_sum_inventory - current_sum_inventory) * 10
        reward += reward_diff
        prev_inventory = next_state['inventory']
        ...
        if done or steps >= max_episode_steps:
            ...
            print(torch.sum(discounted_rewards))
            print(actions_loss)
            print(cameraX_loss)
            print(cameraY_loss)

            loss = actions_loss + cameraX_loss + cameraY_loss
